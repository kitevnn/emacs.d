...
curl_weather_temperature=$(cat ~/.emacs.d/archive/weather | awk -F'[()]' '{print $2}' | grep -oP '[0-9]+' | sed -n '4p')
...
if [ "$curl_weather_temperature" -gt 45 ]; then
        weather_temperature=$(((curl_weather_temperature - 32) * 5 / 9))
fi
